"""
🔐 AUTENTICACIÓN INTERACTIVA CON GITHUB COPILOT
Genera código de 8 caracteres para autenticar
...
class CopilotDeviceAuth:
    """
    Autenticación interactiva con GitHub Copilot
    Genera código de 8 caracteres como VS Code
    """

    def __init__(self):
        self.name = "Copilot Device Auth v1.0"
        self.version = "1.0"
    ...
    def generate_device_code(self) -> Dict:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                print(f"[OK] Código generado")
                return device_info
            else:
                print(f"[!] Error {response.status_code} - usando método alternativo")
            ...
            print(f"[ERROR] {e} - usando método alternativo")
            return self._generate_code_alternative()

    def _generate_code_alternative(self) -> Dict:
        """
        Método alternativo: genera código de 8 caracteres
        """
        print(f"\n[*] Generando código de 8 caracteres...")
        
        # Generar código de 8 caracteres (como VS Code)
    ...
    def show_auth_instructions(self, device_info: Dict) -> None:
        ...
        print(f"1️⃣  COPIA ESTE CÓDIGO (8 caracteres):")
        print(f"   ┌─────────────────────┐")
        print(f"   │  {user_code}  │")
        print(f"   └─────────────────────┘\n")
        
        print(f"2️⃣  ABRE ESTE ENLACE EN TU NAVEGADOR:")
        print(f"   {verification_uri}\n")
        
        print(f"3️⃣  PEGA EL CÓDIGO cuando se te pida\n")
        
        print(f"4️⃣  AUTORIZA la aplicación\n")
        
        print(f"5️⃣  VUELVE AQUÍ - el bot se autenticará automáticamente\n")
    ...
    def wait_for_authentication(self, timeout: int = 900) -> Optional[Dict]:
        ...
        """
        print(f"[*] Esperando autenticación (timeout: {timeout}s)...")
    ...
    def authenticate(self) -> Optional[Dict]:
        ...
        if token:
            ...
            print(f"{'='*100}\n")
            print(f"[OK] Token: {token['access_token'][:20]}...")
            return token
        else:
            print(f"\n{'='*100}")
...
# Singleton
_auth: Optional[CopilotDeviceAuth] = None


def get_copilot_device_auth() -> CopilotDeviceAuth:
    global _auth
    if _auth is None:
        _auth = CopilotDeviceAuth()
    return _auth


if __name__ == "__main__":
    # Ejecutar autenticación interactiva
    auth = get_copilot_device_auth()
